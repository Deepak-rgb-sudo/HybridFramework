...
//method for login
	...
	public static boolean verifyRoleCreation(String rolename,String roledesc,String roletype) throws Throwable
	{
		...
		Thread.sleep(1000);
		driver.findElement(By.xpath(config.getProperty("objRole"))).sendKeys(rolename);
		...
		Thread.sleep(3000);
		//capture alert message
		String roleAlert =driver.switchTo().alert().getText();
		Thread.sleep(3000);
		driver.switchTo().alert().accept();
		String actualAlert ="New Role with id";
		if(roleAlert.toLowerCase().contains(actualAlert.toLowerCase()))
		{
			Reporter.log(roleAlert,true);
			return true;
		}
		else
		{
